const Button = ({ text, className, type, isSuccess, isLoading, disabled }) => {
  return (
    <button
      type={type}
      className={`${className ?? ""} relative z-20 ${isLoading || disabled || isSuccess ? 'opacity-80' : 'cursor-pointer'}`}
      disabled={isLoading || disabled || isSuccess}
    >
      <div className="px-4 py-4 rounded-lg bg-black-200 flex justify-center items-center relative cursor-pointer overflow-hidden group transition-colors duration-300">
        <div
          className={`absolute -right-10 origin-center top-1/2 -translate-y-1/2 
        w-[120%] h-[120%] group-hover:size-10 group-hover:right-10
        rounded-full ${
          isSuccess ? "bg-green-300" : "bg-white-50"
